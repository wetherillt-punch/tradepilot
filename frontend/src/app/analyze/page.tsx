...
import api, { AnalysisResponse, ChartDataResponse } from "@/lib/api";
import dynamic from "next/dynamic";
import type { TradeLevels } from "@/components/CandlestickChart";

// Dynamic import to avoid SSR issues with lightweight-charts
const CandlestickChart = dynamic(() => import("@/components/CandlestickChart"), {
  ssr: false,
  loading: () => (
    <div className="bg-bg-secondary border border-border-primary rounded-lg h-[420px] flex items-center justify-center">
      <span className="text-sm text-text-muted font-mono">Loading chart...</span>
    </div>
  ),
});

/** Parse entry_zone string like "$186.00-$188.50" or "$186.00 - $188.50" into numbers */
function parseEntryZone(zone: string): { low?: number; high?: number } {
  const prices = zone.match(/\$?([\d,.]+)/g);
  if (!prices) return {};
  const nums = prices.map((p) => parseFloat(p.replace(/[$,]/g, ""))).filter((n) => !isNaN(n));
...
export default function AnalyzePage() {
  ...
  // Fetch chart data when we get analysis results
  useEffect(() => {
    if (!result) {
      setChartData(null);
      return;
    ...
    const fetchChart = async () => {
      ...
      try {
        const data = await api.getChartData(result.plan.ticker);
        setChartData(data);
      } catch (e) {
  ...
  // Build trade levels for chart overlay
  const tradeLevels: TradeLevels | undefined = result
    ? (() => {
        const entry = parseEntryZone(result.plan.entry_zone);
        return {
          entryLow: entry.low,
          entryHigh: entry.high,
          stopLoss: result.plan.stop_loss,
          targets: result.plan.targets.map((t, i) => ({
            price: t.price,
            label: `TP${i + 1} $${t.price} (${t.pct_exit}%)`,
          })),
    ...
    <div className="space-y-6 animate-in">
      ...
      {result && (
        <div className="space-y-4 animate-in">
          ...
          {chartLoading ? (
            <div className="bg-bg-secondary border border-border-primary rounded-lg h-[420px] flex items-center justify-center">
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm text-text-muted font-mono">Loading chart data...</span>
              </div>
            </div>
          ) : chartData ? (
            <CandlestickChart
              candles={chartData.candles as any}
              volumes={chartData.volumes as any}
          ...
          {/* Trade Plan */}
          <TradePlanDisplay analysis={result} />
        </div>
      )}
    </div>
  );
